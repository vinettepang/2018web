var player = {
  ...
  watch: {
    percent(newPercent) {
      if (newPercent >= 0 && !this.touch.initiated) { 
        //进度条的宽度
        const barWidth = this.$refs.progressBar.clientWidth * 1 - this.progressBtnWidth - 28
        //偏移的宽度
    ...
    bufferPercent(newBufferPercent) {
      if (newBufferPercent >= 0) {
        //进度条的宽度
        const barWidth = this.$refs.progressBar.clientWidth * 1 - 30
        //缓存的宽度
  ...
  methods: {
    ...
    progressTouchMove(e) {
      ...
      const offsetWidth = Math.min(this.$refs.progressBar.clientWidth * 1 - this.progressBtnWidth - 28, Math.max(0, this.touch.left + deltaX))
